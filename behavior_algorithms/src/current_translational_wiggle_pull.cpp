...
int main(int argc, char** argv) {
    ...
    ros::init(argc,argv,"translational_wiggle_pull");
    ros::NodeHandle nh;
    ...
    ros::Subscriber ft_subscriber = nh.subscribe("current_frame_ft_wrench",1,ft_callback);                       // subscribe to the force/torque sensor data
    ...
    /*
    How to tune params:
    MOVE_DISTANCE: The distance between the end effector and the virtual attractor in the axis of pull but not the rotation, keep positive as it is a pull, increase for greater force, decrease for lesser force
    ...
    WIGGLE_RATE: How many rotations we do per second in Hz
    ...
    */
    ...
    double WIGGLE_RADIUS = 0.005, WIGGLE_RATE = 0.5, WIGGLE_TIME = 5; //! may want to change the radius
    ...
    nh.param("/translational_wiggle_pull/wiggle_time", WIGGLE_TIME, 5.0); 
    ...
    // if(client_start.call(srv)){ // uncomment for time delay
    //     // success
    ...
    */
    while(current_loop <= MAX_LOOPS && !effort_limit_crossed && !freeze_mode){ //! Freeze mode check? 

        //ROS: To get callback data
        ros::spinOnce();

        // Use sin cos and some angle for rotations, and pull down in z
        virtual_attractor.pose.position.x = current_pose.position.x + cos((current_loop * DT * WIGGLE_RATE * 2 * M_PI)) * WIGGLE_RADIUS;
        virtual_attractor.pose.position.y = current_pose.position.y + sin((current_loop * DT * WIGGLE_RATE * 2 * M_PI)) * WIGGLE_RADIUS;
        virtual_attractor.pose.position.z = current_pose.position.z - MOVE_DISTANCE; // predefined value to pull down
